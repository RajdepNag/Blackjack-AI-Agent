...
class Agent:
    ...
    def make_one_transition(self, action):
        ...
        return self.simulator.state, reward
    
    def simulate_till_termination(self, s):
        trajectory = []
        current_state = s
        state_reward = self.simulator.check_reward()
        
        #add rewards for state s0
        trajectory.append((current_state, state_reward))

        while not self.simulator.game_over():

            # after action we are on next state and get next reward
            action = self.default_policy(current_state)
            next_state, state_reward = self.make_one_transition(action)
    ...
    def MC_run(self, num_simulation, tester=False):
        ...
        for simulation in range(num_simulation):
            ...
            # Start of episode - initialize variables

            cur_state = self.simulator.state
            trajectory = self.simulate_till_termination(cur_state)

            # this stores previous reward to go --> move backwards through trajectory then G(s) = R(s) + gamma*prev_rtg
            prev_rtg = 0
            for (s,r) in trajectory[::-1]:
                G = r + DISCOUNT * prev_rtg
                prev_rtg = G
                self.S_MC[s] += G
                self.N_MC[s] += 1
    ...
    def TD_run(self, num_simulation, tester=False):
        ...
        for simulation in range(num_simulation):
            ...
            while s is not None:
                ...
                next_s, next_reward = self.make_one_transition(action) 
    ...
    def Q_run(self, num_simulation, tester=False, epsilon=0.4):
        ...
        for simulation in range(num_simulation):
            ...
            while s is not None:
                ...
                next_s, next_reward = self.make_one_transition(a)

                if next_s is None:
                    self.Q_values[next_s] = [0,0]
            
                self.N_Q[s][a] += 1
                max_Q = max(self.Q_values[next_s][HIT], self.Q_values[next_s][STAND])
                self.Q_values[s][a] += self.alpha(self.N_Q[s][a]) * (reward + DISCOUNT * max_Q - self.Q_values[s][a])
